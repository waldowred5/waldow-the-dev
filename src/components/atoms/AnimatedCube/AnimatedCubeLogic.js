...
for (let i=0; i<6; i++) {
  ...
  // using the lightColours and lightPositions arrays to input arguments
  lights[i] =
...
// Angle increments on each render (100 = increments to complete revolution)

// Ensure model aspect ratio is
// readjusted when screen size and screen aspect ratio change
const render = function() {
  ...
  // Increment theta,
  // and update sphere x and y position based off new theta value
  theta += dTheta;

  // Store trig functions for sphere orbits
  // (MUST BE INSIDE RENDER FUNCTION OR THETA VALUES ONLY GET SET ONCE)
  const trigs = [
